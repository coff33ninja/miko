...
from web.websocket_manager import get_websocket_manager, AnimationEvent, AnimationEventType
...
class Live2DFlaskApp:
    ...
    def _setup_routes(self):
        ...
        def animate():
            ...
            try:
                ...
                result = asyncio.run(self._execute_animation_with_fallback(
        ...
        def _validate_animation_request(self, request) -> Dict[str, Any]:
            ...
            animation_priority = priority_map.get(priority.lower(), AnimationPriority.NORMAL)
        ...
        async def _execute_animation_with_fallback(self, expression: str, intensity: float, 
                                                 duration: float, priority: AnimationPriority, 
                                                 sync_with_audio: bool) -> Dict[str, Any]:
            ...
            try:
                ...
                logger.info(f"Animation triggered: {expression} (intensity: {intensity}, duration: {duration})")
                
                return {
                    'success': True,
                    'animation': self.current_animation,
                    'sequence_id': sequence_id,
                    'websocket_active': self.websocket_loop is not None and not self.websocket_loop.is_closed(),
        ...
        async def _handle_animation_fallback(self, expression: str, intensity: float, 
            ...
            try:
                ...
                if self.websocket_loop and not self.websocket_loop.is_closed():
                    try:
                        ...
                        return jsonify({'error': f'TTS sync failed: {str(sync_error)}'}), 500
                else:
                ...
                return jsonify({
                    ...
                    'estimated_duration': audio_duration or self.animation_sync._estimate_audio_duration(text)
                })
            ...
            try:
                ...
                if self.websocket_loop and not self.websocket_loop.is_closed():
                    try:
                        ...
                        return jsonify({'error': f'Mouth sync failed: {str(sync_error)}'}), 500
                else:
                    return jsonify({'error': 'WebSocket not active'}), 503
        ...
        def generate_token():
            ...
            try:
                ...
                if not self.settings.livekit.api_key or not self.settings.livekit.api_secret:
                ...
                return jsonify({'error': 'Failed to generate token', 'error_type': 'livekit'}), 500
            except Exception as e:
                self.error_logger.log_error(
                    e,
                    component="web_server",
                ...
                health_status = {
                    ...
                    'components': {
                        'web_server': {
                            ...
                            'last_successful_request': datetime.fromtimestamp(self.last_successful_request).isoformat()
                        },
                        'websocket': {
                            'status': 'healthy' if self.websocket_healthy else 'degraded',
                            'active': self.websocket_loop is not None and not self.websocket_loop.is_closed(),
                            'connected_clients': self.websocket_manager.get_connection_count() if hasattr(self.websocket_manager, 'get_connection_count') else 0
                        },
                        'animation_sync': {
                            'status': 'healthy',
                            'active_sequences': len(self.animation_sync.active_sequences) if hasattr(self.animation_sync, 'active_sequences') else 0
                        }
                    }
                }
    ...
    def _start_websocket_server(self):
        ...
        def websocket_thread():
            try:
                ...
                ws_host = self.settings.flask.host
                ws_port = self.settings.flask.port + 1000  # Use port+1000 for WebSocket
    ...
    async def _web_server_health_check(self) -> bool:
        ...
        try:
            ...
            return self.consecutive_failures < 10 and time.time() - self.last_successful_request < 300
        except Exception:
            return False
...
async def _trigger_animation_internal(expression: str, intensity: float, duration: float) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ...
            timeout = aiohttp.ClientTimeout(total=5.0 + attempt * 2)  # Increase timeout on retries
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        raise ValidationError(f"Invalid animation parameters: {response_text}")
                        ...
                        logger.warning(f"Animation trigger failed with status {response.status}")
                        return False
                        
        except asyncio.TimeoutError:
            if attempt < max_retries - 1:
                logger.warning(f"Animation trigger timeout, attempt {attempt + 1}/{max_retries}")
