...
class AnimationSynchronizer:
    ...
    def _register_event_handlers(self) -> None:
        ...
        self.websocket_manager.register_event_handler(
            AnimationEventType.MOUTH_SYNC_START, self._handle_mouth_sync_start
        )
        self.websocket_manager.register_event_handler(
            AnimationEventType.MOUTH_SYNC_STOP, self._handle_mouth_sync_stop
        )
        self.websocket_manager.register_event_handler(
            AnimationEventType.EXPRESSION_CHANGE, self._handle_expression_change
        )
    ...
    async def synchronize_with_tts(
        ...
        try:
            ...
            animation_start_delay = max(
                0, tts_processing_delay - 0.1
            ...
            self.logger.info(
                f"TTS animation synchronized: {sequence_id} ({audio_duration:.2f}s)"
            )
    ...
    async def _create_tts_animation_sequence(
        ...
        mouth_sync_stop_event = AnimationEvent(
            ...
            data={"return_to_expression": self.current_expression},
            sequence_id=sequence_id,
            priority=AnimationPriority.HIGH.value,
        )
    ...
    async def trigger_expression_change(
        ...
        try:
            ...
            if not self.is_transitioning:
                ...
                asyncio.create_task(
                    self._complete_expression_transition(sequence_id, duration)
                )

            self.logger.info(
                f"Expression change triggered: {expression} ({intensity}, {duration}s)"
            )
    ...
    async def _complete_expression_transition(
        self, sequence_id: str, duration: float
    ) -> None:
        """
    ...
    async def start_mouth_sync(
        self, audio_data: Optional[bytes] = None, duration: Optional[float] = None
    ) -> None:
        """
    ...
    async def update_mouth_parameters(
        self, audio_level: float, frequency_data: Optional[List[float]] = None
    ) -> None:
        """
        ...
        mouth_form = (
            self._calculate_mouth_form(frequency_data) if frequency_data else 0.0
        )
    ...
    def _calculate_mouth_form(self, frequency_data: List[float]) -> float:
        ...
        high_freq_ratio = sum(frequency_data[len(frequency_data) // 2 :]) / sum(
            frequency_data
        )
