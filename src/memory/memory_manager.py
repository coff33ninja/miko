...
class MemoryManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            if not self.config.mem0_api_key:
                self.logger.warning("No Mem0 API key provided, using session-only memory")
            ...
            if Memory is None:
                self.logger.error("mem0ai package not available, using session-only memory")
                self._initialized = True
                self.mem0_available = False
                self._register_error_recovery()
                return False
            ...
            result = await self.fallback_manager.execute_with_fallback(
                ...
                context={'retry_operation': self._initialize_mem0_client, 'max_retries': 3}
            )
            ...
            if result.success:
                ...
                self.mem0_available = False
                self.logger.warning("Failed to initialize Mem0, using session-only memory")
    ...
    async def add_memory(self, user_id: str, content: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
        """
        ...
        if self.mem0_available and self._mem0_client:
            ...
            if result.success:
                ...
                self.last_successful_operation = datetime.now()
                await self.recovery_manager.record_success("memory_manager")
                self.logger.debug(f"Stored memory for user {user_id}: {content[:50]}...")
                return True
            else:
            ...
            # Session-only mode
            self.logger.debug(f"Session-only memory add for user {user_id}: {content[:50]}...")
            return True
    
    async def _add_memory_to_mem0(self, user_id: str, content: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            await self.recovery_manager.record_error("memory_manager", MemoryError(
    ...
    def _store_in_session_memory(self, user_id: str, content: str, metadata: Optional[Dict[str, Any]] = None):
        ...
        if len(self._session_memory[user_id]) > self.config.memory_history_limit * 2:
            self._session_memory[user_id] = self._session_memory[user_id][-self.config.memory_history_limit:]
    ...
    async def _search_memories_in_mem0(self, user_id: str, query: str, limit: int) -> List[str]:
    ...
    def _search_session_memories(self, user_id: str, query: str, limit: int) -> List[str]:
    ...
    async def get_user_context(self, user_id: str, query: Optional[str] = None) -> MemoryContext:
        """
        ...
        personality_state = existing_context.personality_state if existing_context else {}
    ...
    async def update_personality_state(self, user_id: str, state_updates: Dict[str, Any]) -> None:
        """
    ...
    async def _prune_session_memory(self, user_id: str) -> None:
        ...
        if len(messages) > limit:
            ...
            self.logger.debug(f"Pruned session memory for user {user_id} to {limit} messages")
    ...
    async def prune_old_memories(self, user_id: str, days_old: int = 30) -> int:
        ...
        try:
            ...
            all_memories = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self._mem0_client.get_all(user_id=user_id)
            )
            ...
            for memory in all_memories:
                if isinstance(memory, dict) and 'created_at' in memory:
                    try:
                        ...
                        elif created_at.tzinfo is None and cutoff_date.tzinfo is not None:
                            cutoff_date = cutoff_date.replace(tzinfo=None)
                        
                        if created_at < cutoff_date and 'id' in memory:
                            await asyncio.get_event_loop().run_in_executor(
                                None,
                                lambda mem_id=memory['id']: self._mem0_client.delete(memory_id=mem_id)
                            )
                            removed_count += 1
                    except (ValueError, TypeError) as e:
                        self.logger.warning(f"Failed to parse created_at for memory {memory.get('id', 'unknown')}: {e}")
                        continue
    ...
    async def delete_user_memories(self, user_id: str) -> bool:
        ...
        try:
            ...
            if self._mem0_client:
                await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: self._mem0_client.delete_all(user_id=user_id)
                )
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'total_messages': sum(len(messages) for messages in self._session_memory.values()),
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        health = {
            ...
            'last_successful_operation': self.last_successful_operation.isoformat() if self.last_successful_operation else None,
            'errors': []
        }
        ...
        try:
            ...
            total_session_entries = sum(len(messages) for messages in self._session_memory.values())
