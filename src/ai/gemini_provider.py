...
from ..error_handling.exceptions import AIProviderError, ContentFilterError, NetworkError
...
from ..error_handling.logging_handler import get_content_filter_logger, get_error_logger
...
class GeminiProvider(AIProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if genai is None:
            raise ImportError("google-generativeai library is required for GeminiProvider")
    ...
    async def rotate_api_key(self) -> bool:
        ...
        while attempts < len(self.api_keys):
            ...
            if self.current_key_index == original_index:
                ...
                expired_keys = [
                    key_idx for key_idx, reset_time in self.rate_limit_reset_times.items()
    ...
    async def generate_response(self, messages: List[Message], personality: str = None, memory_context: Optional['MemoryContext'] = None) -> str:
        ...
        ).then(lambda result: result.result if result.success else self._handle_generation_failure(result))
    ...
    async def _generate_response_internal(self, messages: List[Message], personality: str = None, memory_context: Optional['MemoryContext'] = None) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                conversation_text = self._build_conversation_context(messages, personality, memory_context)
                
                # Generate response with timeout
                response = await asyncio.wait_for(
                    self._make_api_request(conversation_text),
                    timeout=30.0
                )
    ...
    async def _handle_api_error(self, error: Exception, attempt: int, max_retries: int):
        ...
        elif any(term in error_msg for term in ['authentication', 'api key', 'unauthorized']):
    ...
    async def _handle_rate_limit_error(self, attempt: int, max_retries: int):
        ...
        self.rate_limit_reset_times[self.current_key_index] = time.time() + 3600  # 1 hour cooldown
    ...
    async def _handle_network_error(self, error: Exception, attempt: int, max_retries: int):
    ...
    async def _handle_auth_error(self, error: Exception, attempt: int, max_retries: int):
    ...
    async def _handle_content_filter(self, messages: List[Message], conversation_text: str):
    ...
    def _build_conversation_context(self, messages: List[Message], personality: str = None, memory_context: Optional['MemoryContext'] = None) -> str:
        ...
        if memory_context and not self.personality_processor:
            ...
            if context_content:
                context_parts.append(f"System: Context from previous conversations:\n{context_content}")
