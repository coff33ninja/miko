...
class PersonalityProcessor:
    ...
    def __init__(self, personality_prompt: str, enable_content_filter: bool = True):
        ...
        self.anime_patterns = {
            'tsundere_phrases': [
                'B-baka!', 'It\'s not like I...', 'Don\'t get the wrong idea!',
                'Hmph!', 'W-whatever!', 'I-it\'s not for you!'
            ],
            'cute_endings': [
                '(*blush*)', '(>.<)', '(^_^)', '(´∀｀)', '(￣▽￣)',
                '(*´ω｀*)', '(◕‿◕)', '(⌒‿⌒)'
            ...
            'exclamations': [
                'Kyaa!', 'Ehh?!', 'Mou~', 'Ara ara~', 'Sugoi!',
                'Kawaii!', 'Yatta!', 'Ganbatte!'
            ]
        }
    ...
    def inject_personality(self, messages: List[Message], memory_context: Optional['MemoryContext'] = None) -> List[Message]:
        """
        ...
        logger.debug(f"Injected personality prompt and memory context into {len(messages)} messages")
    ...
    def _enhance_anime_style(self, response: str) -> str:
        ...
        if '!' in enhanced and random.random() < 0.3:  # 30% chance
            enhanced = enhanced.replace('!', f'! {random.choice(self.anime_patterns["exclamations"])}', 1)
    ...
    def _analyze_sentiment(self, text: str) -> Tuple[Sentiment, float]:
        ...
        logger.debug(f"Sentiment analysis: {dominant_sentiment.value} (confidence: {confidence:.2f})")
    ...
    def get_personality_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'anime_patterns_count': sum(len(patterns) for patterns in self.anime_patterns.values()),
            'sentiment_patterns_count': sum(len(patterns) for patterns in self.sentiment_patterns.values()),
            'supported_sentiments': [s.value for s in Sentiment],
            'animation_mappings': {s.value: anim for s, anim in self.animation_mappings.items()}
        }


def create_personality_processor(personality_prompt: str, enable_content_filter: bool = True) -> PersonalityProcessor:
    """
