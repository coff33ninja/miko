...
class AIProvider(ABC):
    ...
    async def generate_processed_response(self, messages: List[Message], personality: str = None, memory_context: Optional[MemoryContext] = None):
        """
        ...
        if self.personality_processor:
            ...
            processed_messages = self.personality_processor.inject_personality(messages, memory_context)
            raw_response = await self.generate_response(processed_messages, personality, memory_context)
            return self.personality_processor.process_response(raw_response, self.get_provider_name())
        else:
            # Fallback to basic response
            raw_response = await self.generate_response(messages, personality, memory_context)
