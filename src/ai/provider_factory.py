...
class ProviderFactory:
    ...
    def create_provider() -> AIProvider:
        ...
        personality_processor = ProviderFactory._create_personality_processor(
    ...
    def _create_gemini_provider() -> GeminiProvider:
        ...
        if not api_keys_str:
            raise ValueError(
                "GEMINI_API_KEYS environment variable is required when not using Ollama"
            )
            ...
            if not api_keys_str:
                ...
                try:
                    ...
                    if key_index < 0 or key_index >= len(api_keys):
                        errors.append(
                            f"GEMINI_CURRENT_KEY_INDEX ({key_index}) is out of bounds for {len(api_keys)} keys"
                        )
                except ValueError:
    ...
    def _create_personality_processor(provider_name: str) -> PersonalityProcessor:
        ...
        if provider_name == "ollama":
            enable_content_filter = (
                os.getenv("ENABLE_CONTENT_FILTER", "false").lower() == "true"
            )

        logger.info(
            f"Creating personality processor for {provider_name} with content_filter={enable_content_filter}"
        )
