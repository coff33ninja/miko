...
class FallbackManager:
    ...
    def _register_default_handlers(self):
        ...
        self._fallback_handlers[FallbackStrategy.CACHED_RESPONSE] = self._cached_response_handler
        self._fallback_handlers[FallbackStrategy.SIMPLIFIED_RESPONSE] = self._simplified_response_handler
        self._fallback_handlers[FallbackStrategy.SESSION_ONLY] = self._session_only_handler
        self._fallback_handlers[FallbackStrategy.STATIC_FALLBACK] = self._static_fallback_handler
        self._fallback_handlers[FallbackStrategy.ERROR_MESSAGE] = self._error_message_handler
    
    def register_fallback_chain(self, component: str, strategies: List[FallbackStrategy]):
        """
        ...
        self.logger.info(f"Registered fallback chain for {component}: {[s.value for s in strategies]}")
    ...
    async def execute_with_fallback(
        ...
        try:
            result = await self._execute_operation(primary_operation, operation_args, operation_kwargs)
            ...
            self.logger.warning(f"Primary operation failed for {component}: {primary_error}")
    ...
    async def _execute_fallback_chain(
        ...
        strategies = self._fallback_strategies.get(component, [FallbackStrategy.ERROR_MESSAGE])
        ...
        for strategy in strategies:
            try:
                ...
                if not handler:
                    self.logger.error(f"No handler registered for strategy: {strategy.value}")
                    continue
    ...
    async def _execute_operation(self, operation: Callable, args: tuple, kwargs: Dict[str, Any]) -> Any:
    ...
    async def _retry_handler(self, component: str, error: Exception, context: Dict[str, Any],
                           operation_args: tuple, operation_kwargs: Dict[str, Any]) -> Any:
        ...
        for attempt in range(max_retries):
            try:
                ...
                return await self._execute_operation(operation, operation_args, operation_kwargs)
    ...
    async def _cached_response_handler(self, component: str, error: Exception, context: Dict[str, Any],
                                     operation_args: tuple, operation_kwargs: Dict[str, Any]) -> Any:
        """Return cached response if available."""
        cache_key = context.get('cache_key')
        if not cache_key:
            # Generate cache key from operation args
            cache_key = f"{component}:{hash(str(operation_args) + str(operation_kwargs))}"
    ...
    async def _simplified_response_handler(self, component: str, error: Exception, context: Dict[str, Any],
                                         operation_args: tuple, operation_kwargs: Dict[str, Any]) -> Any:
        ...
        if component == "ai_provider":
            ...
            return {"status": "simplified", "message": "Service temporarily unavailable"}
    
    async def _session_only_handler(self, component: str, error: Exception, context: Dict[str, Any],
                                  operation_args: tuple, operation_kwargs: Dict[str, Any]) -> Any:
    ...
    async def _static_fallback_handler(self, component: str, error: Exception, context: Dict[str, Any],
                                     operation_args: tuple, operation_kwargs: Dict[str, Any]) -> Any:
    ...
    async def _error_message_handler(self, component: str, error: Exception, context: Dict[str, Any],
                                   operation_args: tuple, operation_kwargs: Dict[str, Any]) -> Any:
        ...
        return {
            "error": True,
            "message": error_messages.get(component, "Something went wrong, but I'm still here!"),
            "component": component,
            "can_retry": True
        }
    ...
    def _get_simplified_memory_response(self, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _get_simplified_animation_response(self, context: Dict[str, Any]) -> Dict[str, Any]:
