...
class ComponentHealth:
    ...
    def should_attempt_recovery(self, max_failures: int = 3) -> bool:
        ...
        return (
            ...
            not self.recovery_in_progress and
...
class ErrorRecoveryManager:
    ...
    def _register_default_handlers(self):
        ...
        self._recovery_handlers[RecoveryStrategy.RESTART_SERVICE] = self._restart_service_handler
        self._recovery_handlers[RecoveryStrategy.ROTATE_API_KEY] = self._rotate_api_key_handler
        self._recovery_handlers[RecoveryStrategy.CLEAR_STATE] = self._clear_state_handler
        self._recovery_handlers[RecoveryStrategy.REINITIALIZE] = self._reinitialize_handler
        self._recovery_handlers[RecoveryStrategy.WAIT_AND_RETRY] = self._wait_and_retry_handler
    ...
    async def _attempt_recovery(self, component_name: str) -> RecoveryResult:
        ...
        async with self._recovery_locks[component_name]:
            ...
            if (health.last_recovery_attempt and 
                datetime.now() - health.last_recovery_attempt < self.recovery_cooldown):
            ...
            for strategy in strategies:
                try:
                    ...
                    if success:
                        health.record_success()
                        self.logger.info(f"Recovery successful for {component_name} using {strategy.value}")
    ...
    def _start_health_monitoring(self, component_name: str, health_check_func: Callable):
                ...
                try:
                    ...
                    if is_healthy:
                        ...
                        if health.health_check_failures >= 3:
                            error = Exception(f"Health check failed {health.health_check_failures} times")
                            await self.record_error(component_name, error)
                
                except Exception as e:
                    self.logger.error(f"Health monitoring error for {component_name}: {e}")
    ...
    async def _restart_service_handler(self, component_name: str, error: Exception) -> bool:
    ...
    async def _rotate_api_key_handler(self, component_name: str, error: Exception) -> bool:
    ...
    async def _reinitialize_handler(self, component_name: str, error: Exception) -> bool:
    ...
    async def _wait_and_retry_handler(self, component_name: str, error: Exception) -> bool:
    ...
    def get_recovery_stats(self) -> Dict[str, Any]:
        ...
        for name, health in self._component_health.items():
            stats["components"][name] = {
                ...
                "last_successful_operation": health.last_successful_operation.isoformat() if health.last_successful_operation else None
            }
