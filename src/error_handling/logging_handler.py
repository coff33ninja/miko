...
class ContentFilterLogger:
    ...
    def _sanitize_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        safe_fields = {
            'timestamp', 'session_id', 'request_id', 'model_name',
            'response_length', 'processing_time', 'filter_confidence'
        }
                    ...
                    if "Content filter triggered:" in line:
                        try:
                            ...
                            if json_start != -1:
                                ...
                                incident_time = datetime.fromisoformat(json_data["timestamp"]).timestamp()
                                if incident_time >= cutoff_time:
                                    stats["total_incidents"] += 1
                                    
                                    provider = json_data.get("provider", "unknown")
                                    filter_type = json_data.get("filter_type", "unknown")
                                    user_id = json_data.get("user_id")
                                    
                                    stats["by_provider"][provider] = stats["by_provider"].get(provider, 0) + 1
                                    stats["by_filter_type"][filter_type] = stats["by_filter_type"].get(filter_type, 0) + 1
...
class ErrorLogger:
    ...
    def _sanitize_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sensitive_fields = {
            ...
            'user_input', 'message_content', 'personal_info'
        }
