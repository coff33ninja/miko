...
from src.error_handling.exceptions import (
    AIProviderError, MemoryError, LiveKitError, Live2DError, NetworkError
)
...
class TestGeminiProviderErrorScenarios:
    ...
    async def test_content_filter_logging(self):
        ...
        with patch('src.ai.gemini_provider.genai') as mock_genai:
            ...
            with patch.object(provider, 'content_filter_logger') as mock_logger:
                messages = [Message(role="user", content="inappropriate content", timestamp=datetime.now())]
    ...
    async def test_network_timeout_handling(self):
        ...
        with patch('src.ai.gemini_provider.genai') as mock_genai:
            ...
            # Mock timeout error
            with patch.object(provider, '_make_api_request', side_effect=asyncio.TimeoutError):
...
class TestOllamaProviderErrorScenarios:
    ...
    async def test_connection_error_retry(self):
        ...
        with patch('src.ai.ollama_provider.ollama'):
            ...
            with patch.object(provider, '_make_ollama_request', side_effect=mock_request):
    ...
    async def test_model_not_found_error(self):
        ...
        with patch('src.ai.ollama_provider.ollama'):
            ...
            with patch.object(provider, '_make_ollama_request', side_effect=Exception("Model not found")):
                with pytest.raises(AIProviderError) as exc_info:
                    await provider._generate_response_internal([], None, None)
    ...
    async def test_health_check_with_model_validation(self):
        ...
        with patch('src.ai.ollama_provider.ollama'):
            ...
            with patch.object(provider.client, 'list', return_value={
...
class TestMemoryManagerErrorScenarios:
    ...
    async def test_memory_operation_timeout_handling(self):
        ...
        with patch('src.memory.memory_manager.Memory'):
            ...
            with patch('asyncio.get_event_loop') as mock_loop:
                mock_loop.return_value.run_in_executor.return_value = timeout_operation()
                
                # Should handle timeout and fall back to session memory
                results = await memory_manager.search_memories("test_user", "query")
                assert isinstance(results, list)  # Should return empty list from session fallback
    ...
    async def test_memory_health_check_comprehensive(self):
        ...
        with patch('src.memory.memory_manager.Memory'):
            ...
            memory_manager._session_memory["user1"] = [{"content": "test", "timestamp": datetime.now()}]
            memory_manager._session_memory["user2"] = [{"content": "test2", "timestamp": datetime.now()}]
...
class TestLiveKitAgentErrorScenarios:
    ...
    async def test_ai_provider_timeout_handling(self):
        ...
        llm.ai_provider.generate_response = AsyncMock(side_effect=asyncio.TimeoutError())
    ...
    async def test_memory_error_graceful_handling(self):
        ...
        memory_manager = Mock(spec=MemoryManager)
        memory_manager.store_conversation = AsyncMock(side_effect=MemoryError("Memory failed", "store", "test_user"))
        memory_manager.get_user_context = AsyncMock(side_effect=MemoryError("Memory failed", "get", "test_user"))
    ...
    async def test_animation_trigger_failure_handling(self):
        ...
        with patch.object(llm, 'animation_sync') as mock_sync:
            mock_sync.synchronize_with_tts = AsyncMock(side_effect=Exception("Animation failed"))
            
            with patch('src.agent.livekit_agent.trigger_animation', side_effect=Exception("Fallback failed")):
    ...
    async def test_animation_trigger_network_error_retry(self):
        ...
        with patch('src.web.app.get_settings') as mock_settings:
            ...
            with patch('aiohttp.ClientSession') as mock_session:
                mock_session.return_value.__aenter__.return_value.post.return_value.__aenter__ = mock_post
...
class TestErrorRecoveryIntegration:
    ...
    async def test_component_recovery_coordination(self):
        ...
        recovery_manager.register_component("ai_provider", [RecoveryStrategy.ROTATE_API_KEY])
        recovery_manager.register_component("memory_manager", [RecoveryStrategy.RECONNECT])
        recovery_manager.register_component("web_server", [RecoveryStrategy.RESTART_SERVICE])
        
        # Simulate errors in multiple components
        await recovery_manager.record_error("ai_provider", AIProviderError("Rate limit", "gemini", is_rate_limit=True))
        await recovery_manager.record_error("memory_manager", MemoryError("Connection lost", "search", is_mem0_error=True))
        await recovery_manager.record_error("web_server", NetworkError("Server error", "animate"))
    ...
    async def test_fallback_chain_coordination(self):
        ...
        fallback_manager.register_fallback_chain("ai_provider", [
            FallbackStrategy.RETRY,
            FallbackStrategy.CACHED_RESPONSE,
            FallbackStrategy.SIMPLIFIED_RESPONSE
        ])
        
        fallback_manager.register_fallback_chain("memory_manager", [
            FallbackStrategy.RETRY,
