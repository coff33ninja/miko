...
class TestOllamaProvider:
    ...
    def setup_method(self):
        ...
        self.config = {"model": "llama3", "host": "http://localhost:11434"}
...
class TestGeminiProvider:
    ...
    def test_init_missing_genai(self):
        ...
        with patch("src.ai.gemini_provider.genai", None):
            with pytest.raises(
                ImportError, match="google-generativeai library is required"
            ):
    ...
    def test_init_no_api_keys(self):
        ...
        with patch("src.ai.gemini_provider.genai"):
            with pytest.raises(
                ValueError, match="At least one Gemini API key is required"
            ):
...
class TestProviderFactory:
    ...
    def test_create_gemini_provider_no_keys(self):
        ...
        with pytest.raises(
            ValueError, match="GEMINI_API_KEYS environment variable is required"
        ):
            ProviderFactory.create_provider()
...
class TestPersonalityProcessor:
    ...
    def setup_method(self):
        ...
        self.processor = PersonalityProcessor(
            self.personality_prompt, enable_content_filter=True
        )
    ...
    def test_content_filter_inappropriate(self):
        ...
        should_filter, reason = self.processor._check_content_filter(
    ...
    def test_process_response_filtered_gemini(self):
        ...
        inappropriate_response = (
            "This contains explicit content that should be filtered."
        )
        ...
        assert any(
            indicator in processed.content.lower()
            for indicator in anime_style_indicators
        )
    ...
    def test_process_response_no_filter_ollama(self):
        ...
        processor = PersonalityProcessor(
            self.personality_prompt, enable_content_filter=False
        )
    ...
    def test_get_animation_for_sentiment(self):
        ...
        assert (
            self.processor.get_animation_for_sentiment(self.Sentiment.HAPPY) == "smile"
        )
        assert self.processor.get_animation_for_sentiment(self.Sentiment.SAD) == "sad"
        assert (
            self.processor.get_animation_for_sentiment(self.Sentiment.EMBARRASSED)
            == "blush"
        )
        assert (
            self.processor.get_animation_for_sentiment(self.Sentiment.NEUTRAL) == "idle"
        )
...
class TestProviderWithPersonalityProcessor:
    ...
    async def test_gemini_with_personality_processor(self, mock_genai):
        ...
        config = {"api_keys": ["key1"], "model": "gemini-pro", "current_key_index": 0}
        provider = GeminiProvider(config)
        provider.set_personality_processor(self.processor)
    ...
    def test_factory_creates_provider_with_processor(
        self, mock_processor_class, mock_provider_class
    ):
