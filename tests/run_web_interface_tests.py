...
def check_chromedriver():
    ...
    try:
        result = subprocess.run(
            ["chromedriver", "--version"], capture_output=True, text=True, timeout=5
        )
...
def run_javascript_tests():
    ...
    try:
        ...
        result = subprocess.run(
            ["node", "-c", str(test_file)], capture_output=True, text=True, timeout=10
        )
...
def validate_template():
    ...
    template_file = (
        Path(__file__).parent.parent / "src" / "web" / "templates" / "index.html"
    )
    ...
    try:
        ...
        checks = [
            ...
            (
                "Input mode controls",
                "voice-mode-btn" in content and "text-mode-btn" in content,
            ),
