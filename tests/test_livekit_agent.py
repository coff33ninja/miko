...
from src.config.settings import AppConfig, LiveKitConfig, AIConfig, MemoryConfig, PersonalityConfig, AgentsConfig
...
def mock_ai_provider():
    ...
    provider = Mock()
    provider.generate_response = AsyncMock(return_value="Test response from AI! (*happy*)")
    return provider
...
class TestAnimeAILLM:
    ...
    async def test_llm_initialization(self, mock_config, mock_memory_manager):
        ...
        with patch('src.agent.livekit_agent.ProviderFactory.create_provider') as mock_create:
    ...
    @pytest.mark.asyncio
    async def test_chat_processing(self, mock_config, mock_memory_manager, mock_ai_provider):
        ...
        with patch('src.agent.livekit_agent.ProviderFactory.create_provider') as mock_create:
            ...
            with patch('src.agent.livekit_agent.trigger_animation') as mock_trigger:
                ...
                mock_memory_manager.get_user_context.assert_called_with("test_user", "Hello!")
