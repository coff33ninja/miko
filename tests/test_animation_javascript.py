...
class TestJavaScriptAnimationFiles:
    ...
    def test_animation_controller_structure(self):
        ...
        for method in essential_methods:
            assert method in content, f"Method {method} not found in AnimationController"
    ...
    def test_parameter_mapping_structure(self):
        ...
        for method in essential_methods:
            assert method in content, f"Method {method} not found in Live2DParameterMapping"
    ...
    def test_javascript_syntax_basic(self):
        ...
        for filename in js_files:
            ...
            assert len(function_matches) + len(method_matches) > 0, f"No functions found in {filename}"
    ...
    def test_expression_mapping_completeness(self):
        ...
        required_expressions = [
            'neutral', 'happy', 'sad', 'angry', 'surprised', 'speak'
        ]
        
        for expression in required_expressions:
            assert f"'{expression}'" in content, f"Expression {expression} not found in mapping"
    ...
    def test_parameter_definitions(self):
        ...
        for param in required_parameters:
            assert f"'{param}'" in content, f"Parameter {param} not found in definitions"
    ...
    def test_window_exports(self):
        ...
        for filename, exports in expected_exports.items():
            ...
            for export_name in exports:
                assert f"window.{export_name}" in content, f"Export {export_name} not found in {filename}"
        ...
        if self.expressions_dir.exists():
            ...
            for expr_file in expression_files:
                ...
                try:
                    ...
                    assert False, f"Invalid JSON in expression file {expr_file.name}: {e}"
