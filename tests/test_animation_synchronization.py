...
class TestWebSocketAnimationManager:
    ...
    async def test_animation_event_broadcasting(self, manager, animation_event):
        ...
        manager.clients = {"client1": mock_client1, "client2": mock_client2}
...
class TestAnimationSynchronizer:
    ...
    async def test_tts_synchronization(self, synchronizer, mock_websocket_manager):
        ...
        assert (
            mock_websocket_manager.queue_animation.call_count >= 2
        )  # At least start and stop events
    ...
    @pytest.mark.asyncio
    async def test_expression_change_trigger(
        self, synchronizer, mock_websocket_manager
    ):
...
class TestFlaskAnimationIntegration:
    ...
    def test_mouth_sync_control_endpoint(self, flask_app):
        ...
        response = flask_app.post(
            "/animate/sync/mouth", json={"action": "start", "duration": 3.0}
        )
        ...
        response = flask_app.post("/animate/sync/mouth", json={"action": "stop"})
...
class TestLiveKitIntegration:
    ...
    async def test_agent_animation_trigger(self):
        ...
        llm.animation_sync.synchronize_with_tts = AsyncMock(
            return_value="test-sequence"
        )
        ...
        with patch("src.agent.livekit_agent.trigger_animation") as mock_trigger:
            ...
            chat_ctx.messages = [ChatMessage(role="user", content="Hello there!")]
