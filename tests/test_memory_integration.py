...
class TestMemoryIntegration:
    ...
    async def test_memory_context_creation(self, memory_manager):
        ...
        # Get memory context
        context = await memory_manager.get_user_context(user_id, "What did we talk about?")
    ...
    async def test_memory_context_formatting(self, memory_manager):
        ...
        context = MemoryContext(
            ...
            conversation_history=[
                ...
                Message("assistant", "That's great! What's your favorite genre?", datetime.now())
            ],
            personality_state={}
        )
    ...
    async def test_ollama_with_memory_context(self, ollama_provider, memory_manager):
        ...
        # Create messages
        messages = [
            Message("user", "What does 'konnichiwa' mean?", datetime.now())
        ]
        ...
        with patch('asyncio.get_event_loop') as mock_loop:
            ...
            mock_executor.return_value = {
                'message': {'content': 'Konnichiwa means hello in Japanese! Since you\'re learning, that\'s a great word to know.'}
            }
    ...
    async def test_gemini_with_memory_context(self, gemini_provider):
        ...
        # Create messages
        messages = [
            Message("user", "What's a tsundere character?", datetime.now())
        ]
        ...
        with patch('asyncio.get_event_loop') as mock_loop:
            mock_executor = AsyncMock()
            mock_response = MagicMock()
            mock_response.text = 'A tsundere is a character who acts cold but is actually caring inside!'
    ...
    async def test_conversation_storage_and_retrieval_flow(self, memory_manager):
        ...
        conversations = [
            ...
            ("assistant", "Great! I recommend checking out some shounen anime with romantic subplots."),
            ("user", "What's shounen?"),
        ]
        
        # Store conversation history
        for i, (role, content) in enumerate(conversations[:-1]):  # Store all but the last message
        ...
        assert any("Great! I recommend checking out some shounen anime" in msg for msg in recent_messages)
    ...
    @pytest.mark.asyncio
    async def test_memory_enhanced_response_generation(self, memory_manager, ollama_provider):
        ...
        previous_messages = [
            ...
            ConversationMessage("assistant", "Nice to meet you, Alice!", datetime.now(), user_id),
            ConversationMessage("user", "I love cats", datetime.now(), user_id),
            ConversationMessage("assistant", "Cats are wonderful pets!", datetime.now(), user_id),
        ]
    ...
    async def test_memory_context_with_personality_processor(self, memory_manager):
        ...
        user_id = "test_user"
        personality_prompt = "You are a tsundere anime girl who remembers conversations."
        
        # Create personality processor
        processor = PersonalityProcessor(personality_prompt, enable_content_filter=False)
        ...
        memory_context = MemoryContext(
            ...
            conversation_history=[
                Message("user", "You're really cute when you're flustered", datetime.now()),
                Message("assistant", "B-baka! Don't say things like that!", datetime.now())
            ],
            personality_state={}
        )
        
        # Create current messages
        messages = [Message("user", "Do you remember what I said before?", datetime.now())]
        ...
        memory_system_msg = next((msg for msg in system_messages if "Context from previous conversations" in msg.content), None)
