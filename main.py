...
def validate_environment():
    ...
    if not Path(".env").exists():
        if Path(".env.example").exists():
            ...
            raise ConfigurationError("No .env file found. Copy .env.example to .env and configure your settings.")
        else:
...
async def main():
    ...
    try:
        ...
        logger.info(f"Content Filtering: {'Enabled' if config.content_filter.enable_content_filter else 'Disabled'}")
        logger.info(f"Memory System: {'Mem0' if config.memory.mem0_api_key else 'Session-only'}")
        ...
        print(f"Memory: {'Mem0 enabled' if config.memory.mem0_api_key else 'Session-only'}")
...
async def validate_system_dependencies(config):
    ...
    if not static_dir.exists():
        logger.warning(f"Static models directory {static_dir} does not exist. Creating it...")
